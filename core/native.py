from ctypes import util, CDLL, RTLD_GLOBAL
import ctypes
import os
import platform
...
if system == "Windows":
    ...
    lib = ctypes.CDLL("./libGSPCore.so", winmode=0)
elif system == "Darwin":  # macOS
    lib = ctypes.CDLL("./libGSPCore.dylib", winmode=0)
else:
    raise OSError(f"Unsupported operating system: {system}")

class gsp_list:
    
    lib = CDLL("./libGSPCore.so", winmode=0)

    @staticmethod
    def gsp_list_create_list():
        f_gsp_list_create_list = lib.gsp_list_create_list
        f_gsp_list_create_list.restype = ctypes.c_void_p
        return f_gsp_list_create_list()

    @staticmethod
    def gsp_list_destroy_list(list):
        f_gsp_list_destroy_list = lib.gsp_list_destroy_list
...
class gsp_tree:
    ...
    def gsp_tree_create_root_node():
        f_gsp_tree_create_root_node = lib.gsp_tree_create_root_node
    ...
    def gsp_tree_create_child_node(parent):
        f_gsp_tree_create_child_node = lib.gsp_tree_create_child_node
...
class gsp_window:
    ...
    def gsp_window_poll_events():
        f_gsp_window_poll_events = lib.gsp_window_poll_events
        return f_gsp_window_poll_events()

    @staticmethod
    def gsp_window_is_window_valid(window):
        f_gsp_window_is_window_valid = lib.gsp_window_is_window_valid
    ...
    def gsp_window_set_event_callback(window, callback_handle):
        f_gsp_window_set_event_callback = lib.gsp_window_set_event_callback
...
class gsp_view:
    ...
    def gsp_view_is_view_root(view):
        f_gsp_view_is_view_root = lib.gsp_view_is_view_root
    ...
    def gsp_view_set_window_root(window, view):
        f_gsp_view_set_window_root = lib.gsp_view_set_window_root
    ...
    def gsp_view_add_child(parent, child):
        f_gsp_view_add_child = lib.gsp_view_add_child
    ...
    def gsp_view_set_background_color(view, color):
        f_gsp_view_set_background_color = lib.gsp_view_set_background_color
